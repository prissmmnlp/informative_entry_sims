...
results = foreach(i =c(1:num_cohorts), .export=ls(.GlobalEnv), .packages=c('dplyr','foreach','survival','tranSurv','depend.truncation')) %dopar% {
  ...
  early_cohort_survival_times = rexp(num_early_patients, final_rates[1:num_early_patients])
  
  
  late_cohort_survival_times = rexp(num_late_patients, final_rates[num_early_patients+1:num_patients_per_cohort])
...
results = foreach(i =c(1:num_cohorts), .export=ls(.GlobalEnv), .packages=c('dplyr','foreach','survival','tranSurv','depend.truncation')) %dopar% {
  ...
  final_progression_rates = ifelse(biomarker_statuses == 0, progression_rate, progression_rate*biomarker_rate_ratio)
